        ...
        with open(csv_path, encoding="utf-8-sig", newline="") as f:
            ...
            for row in reader:
                ...
                defaults = {
                    ...
                    "cm_type": clean(row.get("Tipo de CM", "")),
                    "gender": clean(row.get("Sexo", "")),
                    "status": clean(row.get("Status", "")),
                    "performance": clean(row.get("Desempeño", "")),
                    "content_quality": clean(row.get("Calidad del contenido", "")),
                    "appearance": clean(row.get("Apariencia", "")),
                    "is_mother": parse_bool(row.get("Mamá", "")),
                    "content_categories": clean(row.get("Contenido", "")),
                    "follows_stimada": parse_bool(row.get("Siguen a @stimada", "")),
                    "stimada_in_bio": parse_bool(row.get("@stimada en bio", "")),
                    "contract_signed": parse_bool(row.get("Contrato Firmado", "")),
                    "fee_instagram": parse_decimal(row.get("Fee Instagram", "")),
                    "instagram_followers_category": clean(row.get("Categorías # Seguidores Instagram", "")),
                    "instagram_followers": parse_int(row.get("Instagram Seguidores", "")),
                    "instagram_handle": clean(row.get("Instagram", "")),
                    "instagram_link": clean(row.get("Link Instagram", "")),
                    "fee_tiktok": parse_decimal(row.get("Fee Tiktok", "")),
                    "tiktok_followers_category": clean(row.get("Categorías # Seguidores TikTok", "")),
                    "tiktok_followers": parse_int(row.get("TikTok Seguidores", "")),
                    "tiktok_handle": clean(row.get("TikTok", "")),
                    "tiktok_link": clean(row.get("Link perfil Tik tok", "")),
                    "top_size": clean(row.get("Tallaje: \nParte de Arriba", "")),
                    "bottom_size": clean(row.get("Tallaje: \nParte de Abajo", "")),
                    "shoe_size": clean(row.get("Tallaje: \nPie", "")),
                    "height_measurements": clean(row.get("Tallaje: \nAltura", "")),
                    "comments": clean(row.get("Comentarios", "")),
                    "email": clean(row.get("Mail", "")),
                    "phone": clean(row.get("Teléfono", "")),
                    "billing_address": clean(row.get("Dirección de Facturación", "")),
                    "postal_code": clean(row.get("C.P.", "")),
