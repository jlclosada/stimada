...
<template>
  <div class="max-w-5xl animate-fade-up">
    ...
    <div v-if="store.isLoading" class="flex justify-center py-20">
      <div
        class="w-7 h-7 rounded-full border-2 border-gold/20 border-t-gold animate-spin"
      />
    </div>
      ...
      <table class="w-full text-left">
        <thead>
          <tr class="border-b border-border/40">
            <th
              class="px-5 py-3 text-[11px] font-semibold uppercase tracking-wider text-muted/70"
            >
              Marca
            </th>
            <th
        ...
        <tbody>
          ...
          >
            <tr
              class="border-b border-border/20 last:border-0 hover:bg-panel/40 cursor-pointer transition-colors"
              @click="navigate"
            >
              <td class="px-5 py-3.5">
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    <p class="text-sm font-medium text-ink">{{ brand.name }}</p>
    ...
    <div v-else class="text-center py-20">
      ...
      <NuxtLink
        to="/dashboard/brands/new"
        class="inline-flex items-center gap-1.5 text-xs text-gold mt-3 hover:text-gold/80"
      >
        <svg
          class="w-3.5 h-3.5"
          fill="none"
