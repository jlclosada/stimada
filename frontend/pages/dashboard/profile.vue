...
async function handleChangePassword() {
  ...
  try {
    ...
    if (fetchErr?.data) {
      ...
      pwError.value =
        fetchErr.data[firstKey]?.[0] ?? 'Error al cambiar la contraseña.';
    } else {
...
// Client brands
const clientBrands = ref<
  { id: number; brand_id: string; name: string; status: string }[]
>([]);
...
async function fetchClientBrands() {
  ...
  try {
    const data = await $fetch<any>(`${config.public.apiBase}/clients/me/`, {
      headers: { Authorization: `Bearer ${auth.accessToken}` },
    });
  ...
  <div class="max-w-2xl mx-auto animate-fade-up">
    ...
    <div class="mb-8">
      <div class="flex items-center gap-5">
        <div
          class="w-20 h-20 rounded-2xl overflow-hidden border-2 border-border/60 shadow-card flex-shrink-0"
        >
          <img
            v-if="avatarUrl"
            :src="avatarUrl"
          ...
          <div
            v-else
            class="w-full h-full bg-gradient-to-br from-gold/15 to-amber-100 flex items-center justify-center text-gold text-2xl font-semibold"
          >
            {{ initials }}
          </div>
        </div>
        <div>
          ...
          <span
            ...
            :class="
              ROLE_COLORS[auth.user?.role ?? ''] ?? 'text-ink border-border'
            "
          >
    ...
    <div v-if="activeTab === 'personal'" class="space-y-6">
      ...
      <div class="rounded-2xl border border-border/60 bg-white shadow-card p-6">
        ...
        <div class="flex items-center gap-5">
          <div
            class="w-16 h-16 rounded-xl overflow-hidden border border-border/60 flex-shrink-0"
          >
            <img
              v-if="avatarUrl"
              :src="avatarUrl"
            ...
            <div
              v-else
              class="w-full h-full bg-gradient-to-br from-gold/10 to-amber-50 flex items-center justify-center text-gold text-lg font-semibold"
            >
              {{ initials }}
            </div>
          </div>
          <div class="flex-1">
            <div class="flex items-center gap-3">
              <label
                class="px-4 h-9 rounded-xl border border-border/60 text-xs font-medium text-muted hover:text-ink hover:border-ink/20 cursor-pointer transition-all flex items-center gap-1.5"
              >
                <svg
                  class="w-3.5 h-3.5"
                  fill="none"
      ...
      <div class="rounded-2xl border border-border/60 bg-white shadow-card p-6">
        ...
        <form class="space-y-4" @submit.prevent="handleSave">
          <div>
            <label class="block text-[11px] font-medium text-muted mb-1.5"
      ...
      <div
        v-if="isClient"
        class="rounded-2xl border border-border/60 bg-white shadow-card p-6"
      >
        ...
        <div v-if="loadingBrands" class="flex justify-center py-6">
          <div
            class="w-5 h-5 rounded-full border-2 border-gold/30 border-t-gold animate-spin"
          />
        </div>
        ...
        <div v-else-if="clientBrands.length" class="space-y-2">
          ...
          >
            <div
              class="w-9 h-9 rounded-lg bg-indigo-500/10 flex items-center justify-center flex-shrink-0"
            >
            ...
            <span
              ...
              :class="
                brand.status === 'activa'
                  ...
                  : 'text-muted bg-panel border-border/40'
              "
            >
        ...
        <div v-else class="text-center py-6">
          ...
          <p class="text-[11px] text-muted">
            No hay marcas asociadas a tu cuenta.
          </p>
        </div>
      </div>
      ...
      >
        ...
        <div v-else class="text-center py-6">
          ...
          <NuxtLink
            ...
            class="inline-block mt-2 text-[11px] text-gold hover:text-gold/80 font-medium transition-colors"
          >
