...
function toggleAction(action: DashboardData['pending_actions'][number]) {
  if (action.count === 1 && action.items.length === 1) {
    navigateTo(`/projects/${action.items[0].project_id}`);
    return;
  }
  expandedAction.value =
    expandedAction.value === action.type ? null : action.type;
}
...
function getEventsForDate(d: Date) {
  ...
  return data.value.project_ranges.filter(
    (p) => p.start_date === ds || p.service_date === ds || p.end_date === ds,
  );
}
...
async function fetchDashboard() {
  ...
  try {
    const res = await fetch(
      `${config.public.apiBase}/projects/staff_dashboard/`,
      {
...
<template>
  <div class="space-y-6 animate-fade-up">
    ...
    <div class="flex items-center justify-between">
      <div>
        <p
          class="text-[11px] font-semibold uppercase tracking-[0.15em] text-gold mb-1"
        >
          Panel de control
        </p>
    ...
    <template v-else-if="data">
      <!-- Today's Tasks Alert -->
      <div
        v-if="data.todays_tasks.length > 0"
        class="rounded-2xl border border-amber-200 bg-amber-50/60 p-5"
      >
        <div class="flex items-center gap-2 mb-3">
          <div
            class="w-7 h-7 rounded-xl bg-amber-100 flex items-center justify-center"
          >
            <svg
          ...
          <span
            class="ml-auto text-xs font-medium text-amber-600 bg-amber-100 px-2 py-0.5 rounded-full"
          >
          ...
          >
            ...
            <div class="flex-1 min-w-0">
              ...
              <p class="text-xs text-muted">
                {{ task.event }} · {{ task.client_name ?? 'Sin cliente' }}
              </p>
            </div>
            <span
              class="text-[10px] px-2 py-0.5 rounded-md font-medium"
              :class="statusColor(task.status_name)"
            >
              {{ task.status_name }}
            </span>
          </NuxtLink>
      ...
      <div v-if="auth.isAdmin" class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="glass-card p-5 space-y-2">
          <div class="flex items-center gap-2">
            <div
              class="w-8 h-8 rounded-xl bg-gold/10 flex items-center justify-center"
            >
              <svg
            ...
            <p
              class="text-[10px] text-muted uppercase tracking-widest font-medium"
            >
              Proyectos
            </p>
          </div>
          ...
          <p class="text-xs text-muted">
            activos · {{ data.total_projects }} total
          </p>
        </div>
        ...
        <div class="glass-card p-5 space-y-2">
          ...
          <p class="text-xs text-muted">
            {{ data.clients_with_active }} con proyectos activos
          </p>
        </div>
        ...
        <div class="glass-card p-5 space-y-2">
          ...
          <p class="text-xs text-muted">
            {{ data.cms_active }} activas en proyectos
          </p>
        </div>
        ...
        <div class="glass-card p-5 space-y-2">
          ...
          <p class="text-2xl font-bold text-ink">
            {{
              Number(data.total_facturado).toLocaleString('es-ES', {
                minimumFractionDigits: 0,
              })
            }}€
          </p>
          <p class="text-xs text-muted">
            {{ data.completed_projects }} proyectos finalizados
          </p>
        </div>
      </div>
      ...
      <div v-else class="grid grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div class="glass-card p-5 space-y-2">
          <div class="flex items-center gap-2">
            <div
              class="w-8 h-8 rounded-xl bg-orange-50 flex items-center justify-center"
            >
              <svg
      ...
      <!-- Pending Actions -->
      <div
        v-if="data.pending_actions.length > 0"
        class="rounded-2xl border border-orange-200 bg-orange-50/40 p-5"
      >
        <div class="flex items-center gap-2 mb-3">
          <div
            class="w-7 h-7 rounded-xl bg-orange-100 flex items-center justify-center"
          >
            <svg
        ...
        <div class="space-y-3">
          <div v-for="action in data.pending_actions" :key="action.type">
            ...
            >
              ...
              <span class="text-xs text-orange-700 flex-1">{{
                action.label
              }}</span>
              <svg
            ...
            <!-- Expanded detail -->
            <div
              v-if="expandedAction === action.type"
              class="mt-2 ml-2 space-y-1.5 animate-fade-up"
            >
              <NuxtLink
              ...
              >
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <p class="text-[10px] text-muted">
                    {{ item.project_code }}
                    <template v-if="item.cm_name">
                      · CM: {{ item.cm_name }}</template
                    >
                    <template v-else-if="item.client_name">
                      · {{ item.client_name }}</template
                    >
                  </p>
                </div>
                <svg
      ...
      <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div class="xl:col-span-2 glass-card p-6">
          <div class="flex items-center justify-between mb-4">
            ...
            <div class="flex items-center gap-2">
              <button
                @click="prevMonth"
                class="w-7 h-7 rounded-lg border border-border/60 flex items-center justify-center hover:bg-panel transition-colors"
              >
                <svg
                  class="w-3.5 h-3.5 text-muted"
                  fill="none"
              ...
              <span
                class="text-sm font-medium text-ink capitalize min-w-[140px] text-center"
                >{{ monthLabel }}</span
              >
              <button
                @click="nextMonth"
          ...
          <div class="grid grid-cols-7 mb-1">
            <div
              ...
              :key="day"
              class="text-center text-[10px] font-semibold uppercase tracking-wider text-muted py-2"
            >
              {{ day }}
            </div>
          </div>
          ...
          <!-- Calendar grid -->
          <div
            class="grid grid-cols-7 gap-px bg-border/30 rounded-xl overflow-hidden border border-border/40"
          >
            <div
            ...
            >
              <span
                ...
                :class="[
                  ...
                  isToday(day.date)
                    ? 'bg-gold text-white w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold'
                    : '',
                ]"
              >
                {{ day.date.getDate() }}
              ...
              <!-- Events dots -->
              <div
                v-if="day.inMonth && getEventsForDate(day.date).length > 0"
                class="space-y-0.5"
              >
                <div
                  ...
                  :class="
                    ev.service_date === dateStr(day.date)
                      ? 'bg-emerald-100 text-emerald-700'
                      : ev.end_date === dateStr(day.date)
                        ? 'bg-red-100 text-red-600'
                        : 'bg-blue-100 text-blue-600'
                  "
                  :title="`${ev.name} (${ev.client_name ?? ''})`"
                >
          ...
          <div class="flex items-center gap-4 mt-3">
            <div class="flex items-center gap-1.5 text-[10px] text-muted">
              <div
                class="w-2.5 h-2.5 rounded-sm bg-blue-100 border border-blue-200"
              />
              Inicio
            </div>
            <div class="flex items-center gap-1.5 text-[10px] text-muted">
              <div
                class="w-2.5 h-2.5 rounded-sm bg-emerald-100 border border-emerald-200"
              />
              Servicio
            </div>
            <div class="flex items-center gap-1.5 text-[10px] text-muted">
              <div
                class="w-2.5 h-2.5 rounded-sm bg-red-100 border border-red-200"
              />
              Fin
            </div>
          </div>
        ...
        <div class="space-y-6">
          ...
          <div class="glass-card p-5">
            ...
            <div class="space-y-3">
              <div
                v-for="(count, name) in data.status_breakdown"
                :key="name"
                class="flex items-center gap-3"
              >
                <span
                  class="text-[10px] px-2 py-0.5 rounded-md font-medium whitespace-nowrap"
                  :class="statusColor(name as string)"
                >
                  {{ name }}
                </span>
                <div class="flex-1 h-2 bg-panel rounded-full overflow-hidden">
                  <div
                    ...
                    :style="{
                      width: `${(count / (data!.active_projects + data!.completed_projects)) * 100}%`,
                    }"
                  />
                </div>
                <span
                  class="text-xs font-bold text-ink min-w-[20px] text-right"
                  >{{ count }}</span
                >
              </div>
            </div>
          ...
          <div class="glass-card p-5">
            ...
            <div class="space-y-2.5">
              <div
                v-for="(count, name) in data.service_breakdown"
                :key="name"
                class="flex items-center justify-between"
              >
                <span class="text-xs text-muted truncate">{{ name }}</span>
                <span
                  class="text-xs font-bold text-ink bg-panel px-2 py-0.5 rounded-md"
                  >{{ count }}</span
                >
              </div>
            </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          ...
          >
            <div class="text-center min-w-[40px]">
              <p class="text-lg font-bold text-ink leading-none">
                {{ new Date(ev.date).getDate() }}
              </p>
              <p class="text-[9px] uppercase text-muted font-medium">
                {{
      ...
      <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div class="xl:col-span-2 glass-card p-6">
          <div class="flex items-center justify-between mb-4">
            ...
            <NuxtLink
              to="/projects"
              class="text-[11px] text-gold hover:text-gold/80 font-medium"
              >Ver todos →</NuxtLink
            >
          </div>
            ...
            <table class="w-full text-left">
              <thead>
                <tr class="border-b border-border/40">
                  <th
                    class="text-[10px] text-muted uppercase tracking-wider font-medium pb-2 pr-4"
                  >
                    Proyecto
                  </th>
                  <th
                  ...
                  <th
                    class="text-[10px] text-muted uppercase tracking-wider font-medium pb-2"
                  >
                    Servicio
                  </th>
                </tr>
              ...
              <tbody>
                <tr
                  v-for="p in data.recent_projects"
                  :key="p.id"
                ...
                >
                  ...
                  <td class="py-2.5 pr-4">
                    <span
                      class="text-[10px] px-2 py-0.5 rounded-md font-medium"
                      :class="statusColor(p.status_name)"
                    >
                      {{ p.status_name ?? '—' }}
                    </span>
                  </td>
        ...
        <div class="space-y-6">
          ...
          <div class="glass-card p-5">
            <div class="flex items-center justify-between mb-3">
              ...
              <NuxtLink
                ...
                class="text-[10px] text-gold hover:text-gold/80 font-medium"
                >Ver todas</NuxtLink
              >
            </div>
            <div class="space-y-2">
              ...
              >
                <div
                  class="w-7 h-7 rounded-full bg-purple-50 flex items-center justify-center text-[10px] font-bold text-purple-600"
                >
                  {{ cm.name.charAt(0) }}
                </div>
                <div class="flex-1 min-w-0">
                ...
                <span
                  class="text-[10px] font-bold text-gold bg-gold/10 px-2 py-0.5 rounded-full"
                  >{{ cm.projects_count }}</span
                >
              </NuxtLink>
            </div>
          </div>
          ...
          <div class="glass-card p-5">
            ...
            <div class="space-y-2">
              ...
              >
                <div
                  class="w-7 h-7 rounded-full bg-blue-50 flex items-center justify-center text-[10px] font-bold text-blue-600"
                >
                ...
                <span
                  class="text-[10px] font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full"
                  >{{ client.projects_count }} proy.</span
                >
              </NuxtLink>
            </div>
          </div>
