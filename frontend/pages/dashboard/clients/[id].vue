...
async function addBrandToClient() {
  ...
  try {
    await store.createBrand({ client: client.value.id, name: name });
    newBrandName.value = '';
...
async function saveChanges() {
  ...
  try {
    ...
    for (const key of Object.keys(editData.value)) {
      ...
      if (
        editData.value[key] !== (client.value as Record<string, unknown>)[key]
      ) {
    ...
    const updated = await store.updateClient(
...
function openAccountModal() {
  accountForm.value = {
    email: client.value?.billing_email || '',
    full_name: client.value?.name || '',
    password: '',
  };
  accountError.value = '';
...
<template>
  <div>
    ...
    <div v-else-if="error" class="text-center py-20">
      <div
        class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-red-500/5 border border-red-500/20"
      >
        <svg
    ...
    <div v-else-if="client" class="max-w-5xl space-y-6 animate-fade-up">
      ...
      >
        ...
        <span
          class="relative after:absolute after:bottom-0 after:left-0 after:w-0 after:h-px after:bg-ink after:transition-all after:duration-300 group-hover:after:w-full"
        >
          Volver a Clientes
        </span>
      </NuxtLink>
      ...
      <!-- Header Card -->
      <div
        class="relative overflow-hidden rounded-2xl border border-border/60 bg-white shadow-card p-7 transition-all duration-300"
      >
        <div
          class="absolute -top-20 -right-20 w-60 h-60 rounded-full opacity-[0.04] pointer-events-none"
        ...
        <div class="relative flex items-start justify-between gap-6">
          <div class="flex items-center gap-5">
            <div class="relative group/avatar">
              <div
                class="w-16 h-16 rounded-2xl bg-gradient-to-br from-blue-500/20 to-blue-600/10 border border-blue-400/20 flex items-center justify-center text-blue-400 text-xl font-semibold transition-all duration-300 group-hover/avatar:scale-105 group-hover/avatar:shadow-lg group-hover/avatar:shadow-blue-500/10"
              >
            ...
            <div>
              ...
              <div class="flex items-center gap-2.5 mt-2 flex-wrap">
                <span
                  class="text-[11px] text-muted/80 font-mono bg-white/[0.03] px-2 py-0.5 rounded-md border border-white/[0.04]"
                  >{{ formatClientId(client.client_id) }}</span
                >
                <span
                  v-if="client.type_name"
                  class="text-[11px] px-2.5 py-0.5 rounded-full border border-blue-400/15 text-blue-400 bg-blue-400/[0.06] font-medium"
                >
                  {{ client.type_name }}
                </span>
                <span
                ...
                <span
                  v-else
                  class="text-[11px] font-medium px-2.5 py-0.5 rounded-full border text-amber-400 bg-amber-400/[0.06] border-amber-400/15"
                >
                  Contrato pendiente
                </span>
              </div>
          ...
          <div class="flex-shrink-0 flex items-center gap-2">
            <template v-if="canEdit && !isEditing">
              ...
              >
                <span class="flex items-center gap-1.5">
                  <svg
                    class="w-3.5 h-3.5 transition-transform duration-200 group-hover/btn:rotate-[-4deg]"
                    fill="none"
                    stroke="currentColor"
                    stroke-width="2"
                    viewBox="0 0 24 24"
      ...
      <Transition name="modal">
        <div
          v-if="saveError"
          class="rounded-2xl border border-red-500/20 px-5 py-3 text-xs text-red-400 flex items-center gap-2"
      ...
      <template v-if="!isEditing">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          >
            ...
            <div class="space-y-3">
              <div class="flex items-start gap-3">
                <p
                  class="text-[11px] text-muted/70 w-20 flex-shrink-0 pt-0.5 uppercase tracking-wider"
                >
                  Nombre
                </p>
              ...
              <div class="flex items-start gap-3">
                ...
                <p class="text-sm text-ink">{{ client.city || '\u2014' }}</p>
              </div>
              <div class="flex items-start gap-3">
                <p
                  class="text-[11px] text-muted/70 w-20 flex-shrink-0 pt-0.5 uppercase tracking-wider"
                ...
                <p class="text-sm text-ink">{{ client.country || '—' }}</p>
              </div>
            </div>
          </div>
          ...
          >
            ...
            <div class="space-y-3">
              ...
              >
                <svg
                  class="w-3.5 h-3.5 transition-transform duration-200 group-hover/dl:translate-y-0.5"
                  fill="none"
                  stroke="currentColor"
                  stroke-width="2"
                  viewBox="0 0 24 24"
        ...
        <!-- Metadatos -->
        <div
          class="rounded-2xl border border-border/60 bg-white shadow-card px-6 py-4 flex items-center justify-between"
        >
          <div class="flex items-center gap-6">
            <div class="flex items-center gap-2">
              ...
              <span class="text-xs text-ink/80 font-medium">{{
                client.created_by_name || 'Sistema'
              }}</span>
            </div>
        ...
        >
          <div class="flex items-center gap-2">
            ...
            <span
              v-if="client.brands && client.brands.length"
              class="ml-auto text-[10px] text-muted/60 bg-panel px-2 py-0.5 rounded-full border border-border/40"
              >{{ client.brands.length }}</span
            >
          </div>
          <div
            v-if="client.brands && client.brands.length > 0"
            class="space-y-2"
          >
            <NuxtLink
              v-for="brand in client.brands"
              :key="brand.id"
            ...
            >
              <div class="flex items-center gap-3 min-w-0">
                <div
                  class="w-9 h-9 rounded-lg bg-indigo-500/10 border border-indigo-400/20 flex items-center justify-center flex-shrink-0"
                >
                ...
                <div class="min-w-0">
                  <p
                    class="text-sm font-medium text-ink truncate group-hover/item:text-indigo-400 transition-colors duration-200"
                  >
                    {{ brand.name }}
                  </p>
              ...
              <div class="flex items-center gap-2 flex-shrink-0 ml-3">
                <a
                  ...
                  target="_blank"
                  class="text-[11px] text-gold hover:text-gold/80 transition-colors"
                  @click.stop
                >
                  <svg
                    class="w-3.5 h-3.5"
                ...
                <svg
                  class="w-4 h-4 text-muted/40 group-hover/item:text-indigo-400 transition-colors duration-200"
                  fill="none"
                  stroke="currentColor"
                  stroke-width="2"
                  viewBox="0 0 24 24"
      ...
      <template v-if="isEditing">
        <div class="space-y-5">
          <!-- Datos generales -->
          <div
            class="rounded-3xl border border-border/60 bg-white shadow-card p-6 space-y-5"
          >
            <div class="flex items-center gap-2">
              <div
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                >
                <input
                  :value="formatClientId(editData.client_id)"
                  type="text"
                  disabled
          ...
          >
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                >
                <input
                  v-model="editData.billing_email"
                  type="email"
                  class="input-field"
                />
              </div>
              ...
              <div class="md:col-span-2">
                ...
                >
                <input
                  v-model="editData.billing_address"
                  type="text"
                  class="input-field"
                />
              </div>
          ...
          <!-- Contrato -->
          <div
            class="rounded-3xl border border-border/60 bg-white shadow-card p-6 space-y-4"
          >
            <div class="flex items-center gap-2">
              <div
                class="w-7 h-7 rounded-lg bg-gold/10 flex items-center justify-center"
            ...
            <label class="flex items-center gap-3 cursor-pointer group/check">
              <input
                v-model="editData.contract_signed"
                type="checkbox"
                class="w-4 h-4 rounded border-border bg-raised accent-gold"
              />
      ...
      <Transition name="modal">
        ...
        >
          <Transition name="modal-content" appear>
            <div
              class="w-full max-w-md rounded-3xl border border-border/60 p-8 shadow-2xl shadow-black/40"
              style="
            ...
            >
              ...
              <div v-if="accountSuccess" class="space-y-5">
                <div class="flex items-center gap-3">
                  <div
                    class="w-12 h-12 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center"
                  >
                    <svg
                ...
                <div
                  class="rounded-2xl border border-emerald-500/15 p-5 space-y-3"
                  style="background: rgba(16, 185, 129, 0.03)"
                >
                  <div class="flex items-center justify-between">
                    <span
                      class="text-[11px] text-muted uppercase tracking-wider"
                      >Email</span
                    >
                    <span class="text-sm text-ink font-mono">{{
                      accountSuccess.email
                    }}</span>
                  </div>
                  ...
                  <div class="flex items-center justify-between">
                    ...
                    >
                    <span
                      class="text-sm text-ink font-mono bg-white/[0.04] px-2 py-0.5 rounded"
                      >{{ accountSuccess.password }}</span
                    >
                  </div>
                </div>

                <p class="text-xs text-muted/60 leading-relaxed">
                  Comunica estas credenciales al cliente de forma segura. Se
                  recomienda cambiar la contraseña tras el primer acceso.
                </p>
              ...
              <div v-else class="space-y-6">
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p class="text-xs text-muted mt-0.5">
                      El cliente podrá iniciar sesión con estas credenciales
                    </p>
                  </div>
                </div>
                ...
                <form class="space-y-4" @submit.prevent="handleCreateAccount">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      class="input-field"
                    />
                    <p class="text-[10px] text-muted/40 mt-1.5">
    ...
    <Teleport to="body">
      <Transition name="modal">
        ...
        >
          <Transition name="modal-content" appear>
            <div
              class="w-full max-w-sm rounded-3xl border border-red-500/20 p-8 shadow-2xl shadow-black/40"
              style="
                background: linear-gradient(180deg, #161619 0%, #111113 100%);
              "
            >
