    ...
    return [
      {
        ...
        description:
          'Gestiona perfiles, filtros y datos de creadores de contenido',
...
const calendarDays = computed<CalendarDay[]>(() => {
  ...
  for (const range of ranges) {
    ...
    const servicio = range.service_date;
...
<template>
  <div class="max-w-5xl mx-auto px-8 py-12 space-y-10 animate-fade-up">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      >
        <div class="flex items-start gap-4">
          <!-- Icon -->
          <div
            class="w-10 h-10 rounded-xl bg-white/80 border border-white shadow-sm flex items-center justify-center flex-shrink-0 group-hover:scale-105 transition-transform duration-300"
          >
            <svg
              class="w-5 h-5 text-ink/70"
              fill="none"
              stroke="currentColor"
              stroke-width="1.5"
              viewBox="0 0 24 24"
          ...
          <div class="min-w-0 flex-1">
            <h3 class="text-sm font-semibold text-ink group-hover:text-ink/90">
              {{ action.title }}
            </h3>
          ...
          <!-- Arrow -->
          <svg
            class="w-4 h-4 text-muted/40 group-hover:text-ink/50 group-hover:translate-x-0.5 transition-all duration-200 flex-shrink-0 mt-0.5"
            fill="none"
            stroke="currentColor"
            stroke-width="2"
            viewBox="0 0 24 24"
    ...
    <div v-if="isClient && dashboard" class="space-y-6">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <div class="rounded-2xl border border-border/60 bg-white p-5 space-y-2">
          ...
          <p class="text-2xl font-bold text-ink">
            {{ dashboard.total_content_makers }}
          </p>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="rounded-2xl border border-border/60 bg-white p-6">
          ...
          <div class="space-y-3">
            <div
              v-for="(count, name) in dashboard.status_breakdown"
              :key="name"
            ...
            <div
              v-if="!Object.keys(dashboard.status_breakdown || {}).length"
        ...
        <div class="rounded-2xl border border-border/60 bg-white p-6">
          ...
          <div class="space-y-3">
            <div
              v-for="(count, name) in dashboard.service_breakdown"
              :key="name"
              class="flex items-center justify-between"
            >
              <div class="flex items-center gap-2.5">
            ...
            <div
              v-if="!Object.keys(dashboard.service_breakdown || {}).length"
              class="text-xs text-muted text-center py-4"
            >
              Sin datos
            </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <!-- Calendar -->
        <div
          class="lg:col-span-2 rounded-2xl border border-border/60 bg-white p-6"
        >
          <div class="flex items-center justify-between mb-4">
            ...
            <div class="flex items-center gap-1">
              <button
                class="w-7 h-7 rounded-lg border border-border/60 flex items-center justify-center hover:bg-panel/60 transition-colors"
                @click="prevMonth"
              >
                <svg
              ...
              <span
                class="text-xs font-medium text-ink px-2 min-w-[120px] text-center capitalize"
          ...
          <div class="grid grid-cols-7 mb-1">
            <div
              ...
              :key="day"
              class="text-center text-[10px] font-medium text-muted/70 py-1"
            >
              {{ day }}
            </div>
          </div>
          <!-- Day cells -->
          <div
            class="grid grid-cols-7 gap-px bg-border/20 rounded-xl overflow-hidden border border-border/40"
          >
            <div
            ...
            >
              <span
                class="text-[10px] leading-none block mb-0.5"
                :class="{
                  'text-muted/40': !day.inMonth,
                  'text-ink': day.inMonth && !day.isToday,
                  'font-bold text-white bg-gold rounded-full w-4.5 h-4.5 flex items-center justify-center text-[9px]':
                    day.isToday,
                }"
              ...
              <div v-if="day.projects.length" class="space-y-px">
                <div
                  ...
                  :class="
                    proj.type === 'servicio'
                      ? [proj.color.highlight, 'ring-1 ring-white shadow-sm']
                      : proj.color.bg
                  "
                />
                <div
                  v-if="day.projects.length > 3"
                  class="text-[8px] text-muted text-center"
                >
          ...
          <div class="mt-4 space-y-2">
            <div class="flex items-center gap-4">
              <div class="flex items-center gap-1.5">
                <div class="w-6 h-[5px] rounded-sm bg-blue-100" />
                <span class="text-[10px] text-muted"
                  >Duración del proyecto</span
                >
              </div>
              <div class="flex items-center gap-1.5">
                <div
                  class="w-3 h-[5px] rounded-sm bg-blue-500 ring-1 ring-white shadow-sm"
                />
                <span class="text-[10px] text-muted">Fecha de servicio</span>
              </div>
            </div>
            <!-- Project color legend -->
            <div
              v-if="dashboard.project_ranges?.length"
              class="flex flex-wrap gap-x-3 gap-y-1"
            >
              <div
                v-for="range in dashboard.project_ranges.slice(0, 6)"
                :key="range.id"
                class="flex items-center gap-1.5"
              >
                <div
                  class="w-2.5 h-2.5 rounded-sm"
                  :class="getProjectColor(range.id).bg"
                />
                <span class="text-[10px] text-muted truncate max-w-[100px]">{{
                  range.name
                }}</span>
              </div>
            </div>
        ...
        <div class="space-y-4">
          <div class="rounded-2xl border border-border/60 bg-white p-6">
            ...
            <div v-if="dashboard.content_makers?.length" class="space-y-2.5">
              ...
              >
                <div class="w-8 h-8 rounded-lg overflow-hidden flex-shrink-0">
                  ...
                  <div
                    v-else
                    class="w-full h-full bg-gradient-to-br from-gold/10 to-amber-100 flex items-center justify-center text-[10px] font-bold text-gold"
                  >
                    {{
                      cm.name
                        ?.split(' ')
                ...
                <div class="min-w-0 flex-1">
                  <p
                    class="text-xs font-medium text-ink truncate group-hover:text-gold transition-colors"
                  >
                    {{ cm.name }}
                  </p>
                  <p class="text-[10px] text-muted truncate">
                    <span v-if="cm.instagram_handle"
                      >@{{ cm.instagram_handle }}</span
                    >
                    <span v-else>Sin Instagram</span>
                    · {{ cm.projects_count }} proyecto{{
                      cm.projects_count !== 1 ? 's' : ''
                    }}
                  </p>
                </div>
                <svg
                  class="w-3.5 h-3.5 text-muted/30 group-hover:text-gold/60 transition-colors flex-shrink-0"
                  fill="none"
                  stroke="currentColor"
                  stroke-width="2"
                  viewBox="0 0 24 24"
            ...
            <div v-else class="text-center py-8">
              <svg
                class="w-8 h-8 text-muted/20 mx-auto mb-2"
                fill="none"
                stroke="currentColor"
                stroke-width="1.5"
                viewBox="0 0 24 24"
              ...
              <p class="text-[11px] text-muted">
                Aún no tienes Content Makers asignadas
              </p>
            </div>
          </div>
          ...
          <div class="rounded-2xl border border-border/60 bg-white p-6">
            <h3
              class="text-sm font-semibold text-ink mb-4 flex items-center gap-2"
            >
              <svg
            ...
            <div v-if="favoriteCMs.length" class="space-y-2.5">
              ...
              >
                <div class="w-8 h-8 rounded-lg overflow-hidden flex-shrink-0">
                  ...
                  <div
                    v-else
                    class="w-full h-full bg-gradient-to-br from-pink-100 to-pink-50 flex items-center justify-center text-[10px] font-bold text-pink-400"
                  >
                    {{
                      cm.name
                        ?.split(' ')
                ...
                <div class="min-w-0 flex-1">
                  ...
                  <p class="text-[10px] text-muted truncate">
                    <span v-if="cm.instagram_handle"
                      >@{{ cm.instagram_handle }}</span
                    >
                    <span v-if="cm.tiktok_handle">
                      · {{ cm.tiktok_handle }}</span
                    >
                  </p>
                </div>
                <svg
            ...
            <div v-else class="text-center py-8">
              ...
              <p class="text-[11px] text-muted">
                Aún no tienes Content Makers favoritas
              </p>
            </div>
          </div>
        </div>
      ...
      <Teleport to="body">
        ...
        >
          <div
            class="bg-ink text-white rounded-xl px-3 py-2.5 shadow-elevated text-[11px] space-y-1.5 max-w-[220px]"
          >
            <p class="font-medium text-white/70 text-[10px]">
              {{
                new Date(hoveredDay.dateStr).toLocaleDateString('es-ES', {
                  weekday: 'long',
                  day: 'numeric',
                  month: 'long',
                })
            ...
            <div
              v-for="proj in hoveredDay.projects.slice(0, 4)"
              :key="proj.id"
            ...
            >
              <div
                ...
                :class="
                  proj.type === 'servicio'
                    ? proj.color.highlight
                    : proj.color.bg
                "
              />
              <span class="truncate">{{ proj.name }}</span>
              <span class="text-white/50 flex-shrink-0 text-[9px]">{{
                proj.type === 'servicio' ? '★ Servicio' : ''
              }}</span>
            </div>
            <p
              v-if="hoveredDay.projects.length > 4"
              class="text-white/50 text-[9px]"
            >
              +{{ hoveredDay.projects.length - 4 }} más
            </p>
          </div>
      ...
      <!-- Upcoming Events -->
      <div
        v-if="dashboard.calendar_events?.length"
          ...
          >
            <div class="flex items-center gap-3">
              <div
                ...
                :class="
                  event.type === 'servicio' ? 'bg-blue-50' : 'bg-emerald-50'
                "
              >
                <svg
                  v-if="event.type === 'servicio'"
                  class="w-4 h-4 text-blue-600"
                  fill="none"
                  stroke="currentColor"
                  stroke-width="1.5"
              ...
              <div>
                <p class="text-xs font-medium text-ink">{{ event.name }}</p>
                <p class="text-[10px] text-muted">
                  {{
                    event.type === 'servicio' ? 'Fecha servicio' : 'Fecha fin'
                  }}
                  · {{ formatProjectId(event.project_id) }}
                </p>
              </div>
            </div>
            <span class="text-xs font-medium text-ink/70 tabular-nums">{{
    ...
    <!-- Loading dashboard -->
    <div
      v-else-if="isClient && loadingDashboard"
    ...
    <div v-if="isContentMaker" class="space-y-4">
      ...
      <div v-else-if="unreadNotifications.length > 0" class="space-y-4">
        <div
          class="rounded-xl bg-emerald-50 border border-emerald-200/60 px-4 py-3 flex items-center gap-3"
        >
          <div
          ...
          <p class="text-xs text-emerald-800">
            Tienes
            <span class="font-semibold">{{ unreadNotifications.length }}</span>
            notificación{{ unreadNotifications.length !== 1 ? 'es' : '' }} sin
            leer
          </p>
        </div>
        ...
        <div class="flex items-center gap-3">
          ...
          <span
            class="text-[10px] text-muted/60 font-medium uppercase tracking-wider"
        ...
        <div class="space-y-1.5">
          ...
          >
            ...
            >
              <svg
                v-if="notif.notification_type === 'project_cm_request'"
                class="w-4 h-4"
                :class="notif.read ? 'text-muted' : 'text-gold'"
                fill="none"
                stroke="currentColor"
                stroke-width="1.5"
                viewBox="0 0 24 24"
              ...
              <svg
                v-else-if="notif.notification_type === 'briefing_submitted'"
                class="w-4 h-4"
                :class="notif.read ? 'text-muted' : 'text-blue-600'"
                fill="none"
                stroke="currentColor"
                stroke-width="1.5"
                viewBox="0 0 24 24"
            ...
            <svg
              class="w-3.5 h-3.5 text-muted/30 group-hover:text-muted/60 flex-shrink-0 mt-1 transition-colors"
              fill="none"
              stroke="currentColor"
              stroke-width="2"
              viewBox="0 0 24 24"
      ...
      <div
        v-else
        class="text-center py-10 rounded-xl border border-border/40 bg-white"
      >
        <svg
          class="w-8 h-8 text-muted/30 mx-auto mb-3"
          fill="none"
          stroke="currentColor"
          stroke-width="1.5"
          viewBox="0 0 24 24"
