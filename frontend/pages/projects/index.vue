  ...
  <div class="max-w-7xl mx-auto px-8 py-10 space-y-6 animate-fade-up">
    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-sm text-muted mt-1">
          Gestión de proyectos con clientes y content makers
        </p>
      </div>
      <button
        v-if="canCreate"
    ...
    <div class="flex flex-wrap items-center gap-3">
      ...
      <BaseSelect
        ...
        :options="[
          ...
          ...store.filters.service_types.map((t) => ({
            value: t.name,
            label: t.name,
          })),
        ]"
    ...
    >
      ...
      <template v-else-if="store.list.length">
        <table class="w-full text-sm">
          ...
          <tbody>
            ...
            >
              <td
                v-if="!isClient && !isContentMaker"
                class="px-5 py-3.5 text-xs font-mono text-muted"
              >
                {{ formatProjectId(p.project_id) }}
              </td>
              <td class="px-5 py-3.5 font-medium text-ink">{{ p.name }}</td>
              <td v-if="!isClient" class="px-5 py-3.5 text-muted">
                {{ p.client_name }}
              </td>
              <td class="px-5 py-3.5">
                <span
                  v-if="p.service_type_name"
                  class="px-2 py-0.5 rounded-md text-xs bg-blue-50 text-blue-700 font-medium"
                  >{{ p.service_type_name }}</span
                >
              ...
              <td
                v-if="!isContentMaker"
                class="px-5 py-3.5 font-medium text-ink"
              >
                {{ p.total_price }} €
              </td>
              ...
              <td v-if="!isContentMaker" class="px-5 py-3.5 text-xs">
                <span v-if="p.content_maker_name" class="text-muted">{{
                  p.content_maker_name
                }}</span>
                <span
                  v-else
                  class="px-2 py-0.5 rounded-md text-xs font-medium bg-red-50 text-red-600"
                  >Sin content maker</span
                >
              </td>
            </tr>
          </tbody>
      ...
      <div v-else class="px-8 py-16 text-center">
        <div
          class="w-12 h-12 rounded-2xl bg-panel border border-border/60 flex items-center justify-center mx-auto mb-4"
        >
          <svg
            class="w-6 h-6 text-muted/40"
            fill="none"
            stroke="currentColor"
            stroke-width="1.5"
            viewBox="0 0 24 24"
